...
export default function HomePage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setLoadErr("");
      try {
        const data = await listAnnouncements({ status: "open" }); // ยืดหยุ่น แต่ขอ open เป็นค่าเริ่มต้น
  ...
  const Chip = ({ active, onClick, children }) => (
    <button type="button" onClick={onClick}
      className={`btn btn-sm me-2 mb-2 ${active ? "btn-primary" : "btn-outline-secondary"} chip`} style={{ borderRadius: 999 }}>
      {children}
    </button>
  );
  const SkeletonCard = () => (
  ...
  return (
    <div className="min-vh-100" style={{ background: "linear-gradient(180deg,#f7f7fb 0%,#eef1f7 100%)" }}>
      {/* Top Bar — แบบเดียวกันทุกหน้า */}
      <div className="hero-bar" style={{
        height: 72, background: "linear-gradient(90deg, #6f42c1, #8e5cff)",
        position: "sticky", top: 0, zIndex: 1040, boxShadow: "0 4px 16px rgba(111,66,193,.22)",
      }}>
        <div className="container-xxl d-flex align-items-center h-100">
          <div className="d-flex align-items-center">
            <img src="/src/assets/csit.jpg" alt="Logo" className="rounded-3"
          ...
          <div className="ms-auto d-flex align-items-center gap-2">
            ...
            <button className="btn btn-light btn-sm rounded-pill" onClick={handleLogout}>ออกจากระบบ</button>
          </div>
        </div>
      </div>
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-3">
              <div className="card-body d-flex flex-wrap gap-2 align-items-center">
                ...
                <div className="position-relative me-2 flex-grow-1 flex-md-grow-0" style={{ minWidth: 260 }}>
                  <input
                    type="text" className="form-control rounded-pill ps-3"
                    placeholder="ค้นหา (ชื่อประกาศ / อาจารย์ / รายละเอียด)"
                    value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                  />
                </div>
                <button className="btn btn-outline-primary rounded-pill" onClick={() => navigate("/competency/form")}>
                  ข้อมูลสมรรถนะ
                </button>
                <button className="btn btn-outline-secondary rounded-pill" onClick={() => navigate("/profile")}>
                  Profile
                </button>
              </div>
            </div>
                  ...
                  <div className="col-12">
                    ...
                    {Array.isArray(selectedAnnouncement.work_periods) && selectedAnnouncement.work_periods.length > 0 ? (
                      <div className="fw-normal">
                        {selectedAnnouncement.work_periods.map((p, i) => <div key={i}>• {rangeLine(p)}</div>)}
                      </div>
                    ) : (
      ...
      {/* Local styles ให้สอดคล้องทุกหน้า */}
      <style>{`
